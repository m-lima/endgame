...
char *endgame_conf_push(struct EndgameKey key, ngx_str_t discovery_url,
                        ngx_str_t session_name, uint64_t session_ttl,
                        ngx_str_t session_domain, ngx_str_t client_id,
                        ngx_str_t client_secret, ngx_str_t client_callback_url,
...
struct EndgameError endgame_auth_exchange_token(struct EndgameKey master_key,
                                                ngx_str_t query,
                                                const void *request, int pipe,
                                                void *pool);

struct EndgameError endgame_token_decrypt(struct EndgameKey key, ngx_str_t src,
                                          ngx_str_t *email,
                                          ngx_str_t *given_name,
                                          ngx_str_t *family_name,
                                          ngx_str_t *picture, void *pool);
